...
class MEVProtection {
  ...
  async detectMEVActivity(mint) {
    try {
      ...
      // Simplified MEV detection for now
...
class MultiWalletOrchestrator {
  ...
  loadWallets() {
    ...
    // Load additional wallets if provided
    if (WALLET_KEYS) {
    ...
    switch (currentStep) {
      ...
      case SETUP_STEPS.WAITING_DELAY:
        ...
        ctx.reply(
          ...
          { ...getSet '💡 Examples: `1` (single buy), `3`, `5` (max: 10)',
          { ...getSetupMenu(), parse_mode: 'Markdown' }
        );
        break;
...
bot.action('sell_all_execute', async ctx => {
  ...
  try {
    ...
    if (Array.isArray(results)) {
      let txLinks = results.map((tx, i) => `[Tx${i+1}](https://solscan.io/tx/${tx})`).join(' ');
      ctx.reply(
        '✅ **All Tokens Sold!**\n\n' +
...
async function startPumpLoop(ctx) {
  ...
  while (running && !isShuttingDown) {
    try {
      ...
      const baseDeleMs = session.delaySec * 1000;
      const jitter = 0.8 + Math.random() * 0.4;
      const mevDelay = initialMevAnalysis.riskScore > 0.7 ? 2000 : 0;
      const delayMs = Math.max(500, (baseDeleMs * jitter) + mevDelay);
