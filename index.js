...
async function sendPrivateTransaction(transaction, tip = jitoTip || 10000) {
  try {
    ...
    const jitoEndpoint = 'https://mainnet.block-engine.jito.wtf';
    try {
...
bot.command('status', ctx => {
  if (String(ctx.from.id) !== ADMIN) return;
  ctx.reply(showCurrentConfig(), getStatusMenu());
});

bot.command('advanced', ctx => {
  if (String(ctx.from.id) !== ADMIN) return;
  ctx.reply('🛡️ Advanced Features', getAdvancedMenu());
});

bot.command('help', ctx => {
  if (String(ctx.from.id) !== ADMIN) return;
  ctx.reply('Use the menu to configure and run the pump. /setup to start configuration', getMainMenu());
});
...
bot.on('text', async ctx => {
  ...
  if (!currentStep) {
    await ctx.reply('❗ No active setup — press "Setup Configuration" or run /setup', getMainMenu());
    return;
  }
      ...
      case SETUP_STEPS.WAITING_MULTI_BUYS:
        {
          ...
          await ctx.reply('🎉 Setup complete!\n\n' + getSetupSummary(userData), keyboard);
        }
        break;
    ...
    switch (action) {
      ...
      case 'sell_all_execute':
        try {
          ...
          await ctx.reply('❌ Sell failed: ' + (err.message || err), getMainMenu());
        }
        break;
...
async function startPumpLoop() {
  ...
  try {
    await bot.telegram.sendMessage(ADMIN, `🛡️ MEV Analysis: ${initialMev.riskScore.toFixed(2)} (${initialMev.recommendation})`);
  ...
  while (running && !isShuttingDown) {
    ...
    try {
      await bot.telegram.sendMessage(ADMIN, `🔄 Cycle ${cycle} starting - ${buyAmount.toFixed(4)} SOL`);
      ...
      for (let i=0;i<session.multiBuys;i++){
        ...
        try {
          if (session.multiWallet && multiWallet.getActiveWallets().length > 1) {
            ...
            await bot.telegram.sendMessage(ADMIN, `✅ Buy tx: ${tx}`);
          }
        } catch (err) {
          warn('Buy error', err.message || err);
          await bot.telegram.sendMessage(ADMIN, `❌ Buy failed: ${err.message || err}`);
        }
      ...
      if (session.sellPct > 0 && running && !isShuttingDown) {
        try {
          ...
          await bot.telegram.sendMessage(ADMIN, `📈 Sell results:\n${JSON.stringify(sellRes)}`);
        } catch (err) {
          warn('Sell failed', err.message || err);
          await bot.telegram.sendMessage(ADMIN, `❌ Sell failed: ${err.message || err}`);
        }
      }
      ...
      await bot.telegram.sendMessage(ADMIN, `❌ Cycle ${cycle} error: ${err.message || err}`);
    }
  }

  await bot.telegram.sendMessage(ADMIN, '⏹️ Pump stopped');
  log('Pump loop ended');
}
