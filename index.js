...
async function buyToken(mint, solAmount) {
  ...
  const quote    = new RToken(pool.quoteMint, 9, 'SOL', 'SOL');
...
// Sell token percentage back to SOL
...
// === TELEGRAM UI ===
let running = false;
let session = { mint: '', buySol: 0.1, sellPct: 0, delaySec: 2, buyScale: 1.1, multiBuys: 3 };
let multiBuyMode = false;
...
bot.action('set_mint', ctx => {
  ...
  bot.once('text', async ctx2 => {
    if (ctx2.from.id.toString() !== process.env.ADMIN_CHAT_ID) return;
...
['buy_minus', 'buy_plus', 'sell_minus', 'sell_plus', 'delay_minus', 'delay_plus', 'multiBuyToggle'].forEach(action => {
...
bot.action('config_save', ctx => {
  ctx.reply('✅ Pump configuration saved!');
  ctx.answerCbQuery();
});

bot.action('sell_all', async ctx => {
  if (ctx.from.id.toString() !== process.env.ADMIN_CHAT_ID) return;
  if (!session.mint) return ctx.answerCbQuery('Set mint first!');
  try {
...
bot.command('pump', async ctx => {
  ...
  if (!session.mint) return ctx.reply('❌ Set mint first!');
  running = true;
  ...
  while (running) {
    try {
      ...
      await new Promise(res => setTimeout(res, session.delaySec * 1000 * jitter));
    } catch (e) {
...
bot.command('stop', ctx => {
  if (ctx.from.id.toString() !== process.env.ADMIN_CHAT_ID) return;
  running = false;
  ctx.reply('⏹️ Pump will stop after current round.');
});
