...
bot.start(ctx => {
  ...
  ctx.reply(welcomeMsg, { ...mainMenu, parse_mode: 'Markdown' });
});
...
bot.action('main_menu', ctx => {
  ...
  ctx.editMessageText(welcomeMsg, { ...mainMenu, parse_mode: 'Markdown' });
  ctx.answerCbQuery();
});
...
async function startPumpLoop(ctx) {
  ...
  while (running && !isShuttingDown) {
    try {
      cycleCount++;
      await ctx.telegram.sendMessage(ADMIN, `🔄 **Cycle ${cycleCount} Starting**`, { parse_mode: 'Markdown' });
      ...
      for (let i = 0; i < session.multiBuys; i++) {
        ...
        try {
          ...
          await ctx.telegram.sendMessage(ADMIN, `❌ **Buy ${i + 1} Failed:** ${err.message}`, { parse_mode: 'Markdown' });
        }
      ...
      if (session.sellPct > 0 && running && !isShuttingDown) {
        try {
          ...
          await ctx.telegram.sendMessage(ADMIN, `❌ **Sell Failed:** ${err.message}`, { parse_mode: 'Markdown' });
        }
      }
      ...
      await ctx.telegram.sendMessage(ADMIN, `❌ **Cycle ${cycleCount} Error:** ${e.message}`, { parse_mode: 'Markdown' });
    }
  }
...
bot.command('help', ctx => {
  ...
  const helpMsg = [
    ...
    '3. Click "Start Pump" to begin trading',
    '4. Monitor transactions in real-time',
    '5. Click "Stop Pump" when done',
    '',
    '💡 **Tips:**',
    '• Always test with small amounts first',
    '• Monitor your SOL balance',
    '• Use Emergency Sell All if needed'
  ].join('\n');
...
bot.on('message', ctx => {
  ...
  // If not in setup flow and it's not a command, show main menu
...
bot.catch((err, ctx) => {
  ...
  // Send error with menu to keep user interface intact
  if (ctx) {
    try {
      ctx.reply(
