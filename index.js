...
const {
  ...
  WALLET_PRIVATE_KEYS = '' // For future multi-wallet support
} = process.env;
...
// Parse ADMIN_CHAT_ID to handle both string and number formats
...
bot.action('start_pump', adminOnly, async (ctx) => {
...
bot.action('multi_wallet', adminOnly, (ctx) => {
  // Count actual wallets from environment variables
  const walletCount = SOLANA_PRIVATE_KEY ? 1 : 0;
  ctx.reply(`🎭 Multi-Wallet Status: ${walletCount} wallet${walletCount !== 1 ? 's' : ''} active`);
});
...
bot.action('refresh_menu', adminOnly, (ctx) => {
...
const launchBot = async () => {
  try {
    ...
    console.log(`✅ Bot running in production mode at: ${RENDER_EXTERNAL_URL}`);
  
    // Send startup notification to admin
    await bot.telegram.sendMessage(
      adminChatID,
      `🚀 Bot deployed successfully at ${new Date().toLocaleString()}`
    );
  } catch (error) {
    console.error('🚨 Production launch error:', error);
    process.exit(1);
  }
};
