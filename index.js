...
// Track users waiting for mint input
const waitingForMint = new Set();

bot.action('set_mint', ctx => {
  if (ctx.from.id.toString() !== ADMIN) return;
  waitingForMint.add(ctx.from.id);
  ctx.reply('📝 Please send the token contract address (mint).');
  ctx.answerCbQuery();
});

// Handle text messages (including mint input)
bot.on('text', async ctx => {
  if (ctx.from.id.toString() !== ADMIN) return;
  
  // Check if user is waiting to input mint address
  if (waitingForMint.has(ctx.from.id)) {
    waitingForMint.delete(ctx.from.id);
    const mint = ctx.message.text?.trim();
    if (!mint || mint.length < 32) {
      return ctx.reply('❌ Invalid mint address. Please use /config to try again.');
    }
    session.mint = mint;
    ctx.reply(`✅ Mint set to:\n${session.mint}`);
    sendConfig(ctx);
  }
});
